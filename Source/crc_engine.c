/*
 * crc_engine.c - checksum algorithms from GetCRC 1.16 (PD) by Dirk Stöcker
 *
 * Ported into crc.library engine module; LVO wrappers in crc_lvos.c.
 */
...
static void MD5Update(struct md5context *p, const UBYTE *input, ULONG input_length)
{
  ...
  if(input_length >= buffer_space) /* ie. we have enough to fill the buffer */
  {
    ...
    CopyMem((APTR)input, (APTR)(p->buffer + buffer_index), (ULONG)buffer_space);
  ...
  CopyMem((APTR)(input + input_index), (APTR)(p->buffer + buffer_index),
    (ULONG)(input_length - input_index));
}

static void MD5Final(UBYTE digest[16], struct md5context *p)
{
