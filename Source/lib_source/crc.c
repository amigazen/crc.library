...
*/
///
const char * const LibID = "Created by Jim Sunrise from GetCRC 1.16 (10.09.2001) (PD) by Dirk St�cker <stoecker@epost.de>";
...
#include "crc_protos.h"

#if 00
#if 1 // (as separate utility)
/// printall
void printall(const UBYTE *mem, ULONG size)
{
  UBYTE res[16];
  ULONG i;

  printf("Size              : %08ld\n----------------------------\n", size);
� printf("CRC-16 1          :     %04lX  PCompress2, Arc, DMS, ProPack, LhA, Zoo, Shrink\n", DoCRC16_1(mem, size));
  printf("CRC-16 2          :     %04lX  CCITT, XModem, LU\n", DoCRC16_2(mem, size));
  printf("CRC-16 3          :     %04lX  Donald Kindred's CRC\n", DoCRC16_3(mem, size));
  printf("CRC-16 4          :     %04lX  old Zoom, CompDisk\n", DoCRC16_4(mem, size));
...
� printf("CRC-32 2          : %08lX  Zip, GZip, LZX, RAR, Arj\n", DoCRC32_2(mem, size));
  ...
  printf("CHS-16 1          :     %04lX  PowerPacker Passwords\n", DoCHS16_1(mem, size));
  printf("CHS-32 1M (WRAP)  : %08lX  Olaf Barthel's CRC, Bootblock of FFS\n", DoCHS32_1M(mem, size));
  printf("CHS-32 1I         : %08lX\n", DoCHS32_1I(mem, size));
  printf("CHS-32 2          : %08lX  LightFileSystem\n", DoCHS32_2(mem, size));

  /* EOR methods */
  printf("EOR Byte          :       %02lX\n", DoEORB(mem, size));
  printf("EOR Word (M/I)    :     %04lX/    %04lX", DoEORWM(mem, size), DoEORWI(mem, size));
  if(size & 1)
    printf(" (+%02lX)", mem[size-1]);
  printf("\n");
  ...
  if((fh = fopen(argv[1], "rb")))
  {
    ...
    if(!fseek(fh,0,SEEK_END) && (size = ftell(fh)) != EOF && !fseek(fh,0,SEEK_SET))
    {
      ...
      if((mem = (UBYTE *) malloc(size+3)))
      {
        if(fread(mem, size, 1, fh) == 1)
        {
          ...
          if(argc == 3)
          {
            ...
            {
            case 161:  printf("%04lX", DoCRC16_1(mem, size)); break;
            case 162:  printf("%04lX", DoCRC16_2(mem, size)); break;
            case 163:  printf("%04lX", DoCRC16_3(mem, size)); break;
            case 164:  printf("%04lX", DoCRC16_4(mem, size)); break;
      ...
      case 322:  printf("%08lX", DoCRC32_2(mem, size)); break;
            ...
            case 401:  printf("%04lX", DoCHS16_1(mem, size)); break;
...
///
#else // (as internal function)
/// checkCRC
void checkCRC(char *filename, const int type_of_check, UBYTE *res)
{
  FILE *fh;
...
ULONG DoCHS32_1(ULONG *Mem, LONG Size)
{
  ULONG WRAP = 0, oldWRAP;
...
/* other, less efficent method
void MakeCRC16R(UWORD *buf, UWORD ID)
{
  UWORD i, j, k, l;
...
/* inline method
UWORD DoCRC16_1(const UBYTE *Mem, LONG Size)
{
  UWORD CRC = 0;
  ULONG i;
  UBYTE c;
...
/* inline method
UWORD DoCRC16_2(const UBYTE *Mem, LONG Size)
{
  UWORD CRC = 0;
  ULONG i;
  UWORD c;
...
void MakeCRC16(UWORD * buf, UWORD ID)
{
  UWORD i, j, k;
...
void MakeCRC32R(ULONG * buf, ULONG ID)
{
  ULONG i, j, k;
...
/* table from MakeCRC32R(buf, 0x04C11DB7); */

static const ULONG table32R[256] = {
...
/* table from MakeCRC32(buf, 0xEDB88320); */

static const ULONG table32_[256] = {
...
/* table from MakeCRC16(buf, 0xA001); */

static const UWORD table16_[256] = {
...
/* table from MakeCRC16R(buf, 0x1021); */

static const UWORD table16R[256] = {
...
//    CRC = buf[(CRC ^ *(Mem++)) & 0xFF] ^ (CRC >> 8);
    CRC = table32_[(CRC ^ *(Mem++)) & 0xFF] ^ (CRC >> 8);

  return CRC;
}

/* GZip, Zip, LZX */
ULONG __asm LIBDoCRC32_2(
  register __a0 const UBYTE *Mem,
  register __d0 LONG Size)
{
...
/* GPatch */
ULONG __asm LIBDoCRC32_4(
  register __a0 const UBYTE *Mem,
  register __d0 LONG Size)
{
  ULONG CRC = 0, highbit, newbit;
  UBYTE l, bytevar;
...
//    CRC = buf[(CRC >> 24) ^ *(Mem++)] ^ (CRC << 8);
    CRC = table32R[(CRC >> 24) ^ *(Mem++)] ^ (CRC << 8);

  return ~CRC;
}

/* BZip2 inverted */
ULONG __asm LIBDoCRC32_6(
  register __a0 const UBYTE *Mem,
  register __d0 LONG Size)
{
  ULONG CRC = 0xffffffff;
...
//  MakeCRC16(buf, 0xA001);
  ...
  while(Size--)
//    CRC = buf[(CRC ^ *(Mem++)) & 0xFF] ^ (CRC >> 8);
    CRC = table16_[(CRC ^ *(Mem++)) & 0xFF] ^ (CRC >> 8);

  return CRC;
}

UWORD __asm LIBDoCRC16_2(
  register __a0 const UBYTE *Mem,
  register __d0 LONG Size)
{
  UWORD CRC = 0;
...
//    CRC = buf[((CRC>>8) ^ *(Mem++)) & 0xFF] ^ (CRC<<8);
    CRC = table16R[((CRC>>8) ^ *(Mem++)) & 0xFF] ^ (CRC<<8);

  return CRC;
}

/* Donald Kindred's CRC */
UWORD __asm LIBDoCRC16_3(
  register __a0 const UBYTE *Mem,
  register __d0 LONG Size)
{
  UWORD CRC = 0;
...
//    CRC = buf[((CRC>>8) & 0xFF)] ^ ((CRC << 8) ^ *(Mem++));
    CRC = table16R[((CRC>>8) & 0xFF)] ^ ((CRC << 8) ^ *(Mem++));
...
ULONG __asm LIBDoCHS32_1M(
  register __a0 const UBYTE *Mem,
  register __d0 LONG Size)
{                 
  ULONG WRAP = 0, oldWRAP, i;
...
static void MD5Update(struct md5context *, const UBYTE *, ULONG);
static void MD5Final(UBYTE [16], struct md5context *);
...
/* Encodes input (ULONG) into output (UBYTE). Assumes len is a multiple of 4. */
static void md5_encode(UBYTE *output, ULONG *input, ULONG len)
{
  ULONG i, j;
...
/* Decodes input (UBYTE) into output (ULONG). Assumes len is a multiple of 4. */
static void md5_decode(ULONG *output, const UBYTE *input, ULONG len)
{
  ULONG i, j;
...
static ULONG md5_rotateleft(ULONG x, ULONG n) { return (x << n) | (x >> (32 - n)); }

/* F, G, H and I are basic MD5 functions. */
static ULONG MD5_F(ULONG x, ULONG y, ULONG z) { return (x & y) | (~x & z); }
static ULONG MD5_G(ULONG x, ULONG y, ULONG z) { return (x & z) | (y & ~z); }
static ULONG MD5_H(ULONG x, ULONG y, ULONG z) { return x ^ y ^ z; }
static ULONG MD5_I(ULONG x, ULONG y, ULONG z) { return y ^ (x | ~z); }

/* FF, GG, HH, and II transformations for rounds 1, 2, 3, and 4.
   Rotation is separate from addition to prevent recomputation. */
static void MD5_FF(ULONG* a, ULONG b, ULONG c, ULONG d, ULONG x, ULONG s, ULONG ac)
{
  *a += MD5_F(b, c, d) + x + ac;
  *a = md5_rotateleft(*a, s) + b;
}

static void MD5_GG(ULONG* a, ULONG b, ULONG c, ULONG d, ULONG x, ULONG s, ULONG ac)
{
  *a += MD5_G(b, c, d) + x + ac;
  *a = md5_rotateleft(*a, s) + b;
}

static void MD5_HH(ULONG* a, ULONG b, ULONG c, ULONG d, ULONG x, ULONG s, ULONG ac)
{
  *a += MD5_H(b, c, d) + x + ac;
  *a = md5_rotateleft(*a, s) + b;
}

static void MD5_II(ULONG* a, ULONG b, ULONG c, ULONG d, ULONG x, ULONG s, ULONG ac)
{
  *a += MD5_I(b, c, d) + x + ac;
  *a = md5_rotateleft(*a, s) + b;
}
...
static void transform(struct md5context *p, const UBYTE block[64])
{
  ULONG a = p->state[0], b = p->state[1], c = p->state[2], d = p->state[3], x[16];
...
static void MD5Update(struct md5context *p, const UBYTE *input, ULONG input_length)
{
  ULONG input_index, buffer_index;
  ULONG buffer_space;                /* how much space is left in buffer */

  /* Compute number of bytes mod 64 */
  buffer_index = (ULONG)((p->count[0] >> 3) & 0x3F);

  /* Update number of bits */
  if((p->count[0] += ((ULONG)input_length << 3))<((ULONG)input_length << 3))
    p->count[1]++;

  p->count[1] += ((ULONG)input_length >> 29);
  ...
  if(input_length >= buffer_space) /* ie. we have enough to fill the buffer */
  {
    ...
    memcpy(p->buffer + buffer_index, input, (size_t)buffer_space);
  ...
  memcpy(p->buffer + buffer_index, input + input_index, (size_t)(input_length - input_index));
}
...
void __asm LIBmd5sum(
  register __a0 const UBYTE *Mem,
  register __d0 LONG Size,
  register __a1 UBYTE md5sum[16])
{
  struct md5context m;
